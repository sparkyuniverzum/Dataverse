...
RESERVED_MINERAL_METADATA_KEYS = frozenset(
    {
        "table",
        "table_id",
        "table_name",
...
def _normalize_reserved_keys(reserved_keys: Collection[str] | None) -> set[str]:
    source = reserved_keys if reserved_keys is not None else RESERVED_MINERAL_METADATA_KEYS
    return {str(key or "").strip().lower() for key in source if str(key or "").strip()}


def normalize_civilization_metadata_patch(
    metadata: Mapping[str, Any] | None,
    *,
    reserved_keys: Collection[str] | None = None,
) -> dict[str, Any]:
