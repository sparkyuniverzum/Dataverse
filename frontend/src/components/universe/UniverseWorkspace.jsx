...
export default function UniverseWorkspace({ galaxy, onCreateGalaxy, onBackToGalaxies, onLogout, minimalShell = false }) {
  const {
    level,
    selectedTableId,
    selectedAsteroidId,
  ...
  const minimalWorkspace = minimalShell === true;
  ...
  const stageOneAllowNavigator =
    !minimalWorkspace && (!stageOneImmersiveMode || onboardingMachineStepRank >= ONBOARDING_STEP_RANK[OnboardingSteps.STEP_SCHEMA]);
  const stageOneAllowGuidePanel = !minimalWorkspace && !stageOneImmersiveMode;
  const stageOneAllowDetailPanel =
    !minimalWorkspace &&
    (!isStageOneExclusive || !stageOneImmersiveMode || onboardingMachineStepRank >= ONBOARDING_STEP_RANK[OnboardingSteps.STEP_CALCULATIONS]);
  const stageOneAllowSupportActions =
    !minimalWorkspace && (!stageOneImmersiveMode || onboardingMachineStepRank >= ONBOARDING_STEP_RANK[OnboardingSteps.STEP_SCHEMA]);
  ...
  const showGuidePanel =
    !minimalWorkspace &&
    stageOneAllowGuidePanel &&
    !quickGridOpen &&
    (!selectedAsteroid && !selectedBond || hasHierarchyIssues || onboardingProgress < 3);
      ...
      {minimalWorkspace ? (
        ...
        >
          ...
          <div style={{ display: "flex", gap: 6, flexWrap: "wrap" }}>
            ...
            <span style={hudBadgeStyle}>Mesice {asteroidNodes.length}</span>
            <span style={hudBadgeStyle}>Vazby {enrichedAsteroidLinks.length}</span>
          </div>
          <div style={{ fontSize: "var(--dv-fs-xs)", opacity: 0.82 }}>
            {selectedSemantic?.planetName
              ? `Aktivni planeta: ${selectedSemantic.planetName}`
              : "Vyber planetu kliknutim v prostoru."}
          </div>
          <div style={{ display: "grid", gridTemplateColumns: "1fr 1fr", gap: 6 }}>
            ...
            <button
              type="button"
              onClick={() => {
                void loadUniverse().catch((loadError) => setError(loadError.message || "Refresh failed"));
              }}
              disabled={loading}
          ...
          {error ? (
            <div style={{ fontSize: "var(--dv-fs-xs)", color: "#ffb7c9", borderTop: "1px solid rgba(255, 134, 170, 0.22)", paddingTop: 6 }}>
              {error}
            </div>
          ) : null}
        </aside>
